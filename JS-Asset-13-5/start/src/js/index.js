...
const getFormContents = () => {
  ...
  const extraFieldsValidation =
    productType === "book"
      ? author.value !== "" && isbn.value !== ""
      : ram.value !== "" && storage.value !== "" && color.value !== "";
  const extraFieldsVal =
    productType === "book"
      ? [author.value, isbn.value, genre.value]
      : [os.value, ram.value, storage.value, color.value];
